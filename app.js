...
function generateErrorLog() {
    if (errorLog.length === 0) {
        alert('No errors logged! Everything is working fine.');
        return;
    }
    const logText = errorLog.map(e =>
        `[${e.time}] ${e.context}\n  ${e.message}${e.stack ? '\n  Stack: ' + e.stack.split('\n').slice(0,3).join('\n  ') : ''}`
    ).join('\n\n');
    const fullLog = `MY CINEMA TRACKER — ERROR LOG\nGenerated: ${new Date().toISOString()}\nApp Version: v5.2\nErrors: ${errorLog.length}\n${'='.repeat(40)}\n\n${logText}`;
...
const TZ_OFFSETS = {
    ...
    'America/Sao_Paulo':-3,'Pacific/Auckland':12
};

function getUtcOffset(timezone) {
    if (!timezone) return 0;
    if (TZ_OFFSETS[timezone] !== undefined) return TZ_OFFSETS[timezone];
    try {
        const now = new Date();
        const utc = now.toLocaleString('en-US', { timeZone: 'UTC' });
        const local = now.toLocaleString('en-US', { timeZone: timezone });
        return (new Date(local) - new Date(utc)) / 3600000;
    } catch(e) { return 0; }
}

function convertToGhanaTime(timeStr, timezone) {
    if (!timeStr) return null;
    const [hours, minutes] = timeStr.split(':').map(Number);
    const offset = getUtcOffset(timezone);
    let ghanaHours = hours - offset;
    if (ghanaHours < 0) ghanaHours += 24;
    if (ghanaHours >= 24) ghanaHours -= 24;
    const h12 = ghanaHours % 12 || 12;
    const ampm = ghanaHours >= 12 ? 'pm' : 'am';
    const mins = minutes > 0 ? `:${String(minutes).padStart(2,'0')}` : '';
    return `${h12}${mins}${ampm}`;
}

function getGhanaAirHour(timeStr, timezone) {
    if (!timeStr) return 0;
    const [hours, minutes] = timeStr.split(':').map(Number);
    const offset = getUtcOffset(timezone);
    let ghanaHours = hours - offset + (minutes || 0) / 60;
    if (ghanaHours < 0) ghanaHours += 24;
    if (ghanaHours >= 24) ghanaHours -= 24;
    return ghanaHours;
}
...
async function fetchAirTimeData(show) {
    if (show.air_time_data?.source && show.air_time_data.source !== 'default' && show.air_time_data?.fetched_at) {
        const age = Date.now() - new Date(show.air_time_data.fetched_at).getTime();
        if (age < 30 * 24 * 60 * 60 * 1000) return show.air_time_data;
    }
    ...
    // Try TVMaze
    try {
        const searchUrl = `${TVMAZE_BASE}/singlesearch/shows?q=${encodeURIComponent(show.title)}`;
        ...
        if (response.ok) {
            ...
            if (data?.schedule?.time) {
                airData = {
                    ...
                    timezone: data.network?.country?.timezone || data.webChannel?.country?.timezone || 'UTC',
                    day: data.schedule.days?.[0] || null,
                    source: 'tvmaze',
                    fetched_at: new Date().toISOString()
                };
            }
        }
    } catch(e) { logError('TVMaze fetch', e); }

    // Jikan fallback for anime
    if (!airData && show.is_anime) {
        try {
            await new Promise(r => setTimeout(r, 500));
            const searchUrl = `${JIKAN_BASE}/anime?q=${encodeURIComponent(show.title)}&limit=1`;
            const response = await fetch(searchUrl);
            if (response.ok) {
                const data = await response.json();
                ...
                if (anime?.broadcast?.time) {
                    airData = {
                        ...
                        day: anime.broadcast.day?.replace(/s$/,'') || null,
                        source: 'jikan',
                        fetched_at: new Date().toISOString()
                    };
                }
            }
        } catch(e) { logError('Jikan fetch', e); }
    }
...
// ===== SETTINGS =====
function toggleSettingsGroup(header) { const content = header.nextElementSibling; const arrow = header.querySelector('.settings-arrow'); content.classList.toggle('open'); arrow.classList.toggle('open'); }
function toggleImportSection() { const content = document.getElementById('import-content'); const arrow = document.getElementById('import-arrow'); if (!content) return; const isOpen = content.style.display !== 'none'; content.style.display = isOpen ? 'none' : 'block'; if (arrow) arrow.textContent = isOpen ? '▶' : '▼'; }
...
function setupPullToRefresh() {
    ...
    container.addEventListener('touchstart', (e) => {
        if (window.scrollY === 0 && !document.body.classList.contains('modal-open')) { pullStartY = e.touches[0].clientY; pulling = true; }
    }, { passive: true });
    ...
    container.addEventListener('touchend', async () => {
        ...
        if (indicator.classList.contains('visible')) {
            ...
            await loadMyList();
            setTimeout(() => { indicator.classList.remove('visible'); indicator.style.top = '-60px'; indicator.querySelector('span').textContent = 'Release to refresh...'; }, 800);
...
function setupRealtimeListeners() {
    let debounceTimer;
    const debouncedLoad = () => { clearTimeout(debounceTimer); debounceTimer = setTimeout(() => { if (!isLoading) loadMyList(); }, 5000); };
...
function setupAutoSync() {
    ...
    if (!lastSync || Date.now() - parseInt(lastSync) > twentyFourHours) setTimeout(() => syncAllAiringShows(true), 8000);
    setInterval(() => { const last = localStorage.getItem('lastEpisodeSync'); if (!last || Date.now() - parseInt(last) > twentyFourHours) syncAllAiringShows(true); }, 3600000);
}
    ...
    document.querySelectorAll('.sub-tab-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (tabId === 'anime-upcoming' || tabId === 'tv-upcoming') {
                ...
                if (!cacheTime || Date.now() - parseInt(cacheTime) > 24*60*60*1000) loadSectionCalendar(section);
                else { const cached = localStorage.getItem(cacheKey); if (cached) { try { displayCalendarFromCache(section, JSON.parse(cached)); } catch(e) { loadSectionCalendar(section); } } else loadSectionCalendar(section); }
            }
        });
    });
}
...
function generateIncrementalTimestamps(count, isAnime) {
    ...
    const timestamps = [];
    for (let i = 0; i < count; i++) timestamps.push(new Date(now.getTime() - (count - 1 - i) * gapMinutes * 60000).toISOString());
    return timestamps;
}

// ===== ENRICH / SYNC / AIR TIMES =====
async function enrichLibrary() {
    ...
    if (needsEnrichment.length === 0) { statusEl.innerHTML = `<p style="color:var(--green);">✓ All up to date!</p>`; return; }
    statusEl.innerHTML = `<p style="color:var(--accent);">Enriching ${needsEnrichment.length}...</p>`;
    let done = 0, failed = 0;
    for (const item of needsEnrichment) {
        ...
        try {
            ...
            if (!item.genres || item.genres.length === 0) {
                ...
                enrichData.networks = item.type === 'tv' ? (det.networks||[]).map(n => n.name) : (det.production_companies||[]).map(n => n.name);
            }
            if (!item.year) { const ds = item.type === 'tv' ? det.first_air_date : det.release_date; if (ds) enrichData.year = parseInt(ds.substring(0,4)); }
            if (Object.keys(enrichData).length > 0) { await updateDoc(doc(db, item.type === 'movie' ? 'movies' : 'series', item.docId), enrichData); Object.assign(item, enrichData); }
            done++; await new Promise(r => setTimeout(r, 250));
        } catch(e) { failed++; logError('Enrich', e); }
    }
    saveTmdbCache();
    statusEl.innerHTML = `<p style="color:var(--green);">✓ ${done} enriched${failed>0?`, ${failed} failed`:''}.</p>`;
    await loadMyList();
}

async function fetchAllAirTimes() {
    const statusEl = document.getElementById('settings-action-status');
    const shows = myList.filter(i => i.type === 'tv' && ['Returning Series','In Production'].includes(i.tmdb_status) && (!i.air_time_data || i.air_time_data.source === 'default'));
    if (shows.length === 0) { if (statusEl) statusEl.innerHTML = `<p style="color:var(--green);">✓ All air times up to date!</p>`; return; }
    if (statusEl) statusEl.innerHTML = `<p style="color:var(--accent);">Fetching air times for ${shows.length} shows...</p>`;
    let done = 0;
    for (const show of shows) {
        try { if (statusEl) statusEl.innerHTML = `<p style="color:var(--accent);">Air time ${done+1}/${shows.length}: ${show.title}</p>`; await fetchAirTimeData(show); done++; await new Promise(r => setTimeout(r, 600)); } catch(e) { logError('Fetch air time', e); }
    }
    if (statusEl) statusEl.innerHTML = `<p style="color:var(--green);">✓ Air times for ${done} shows!</p>`;
}
...
async function syncAllAiringShows(silent = false) {
    ...
    if (!silent && statusEl) statusEl.innerHTML = `<p style="color:var(--accent);">Syncing ${shows.length}...</p>`;
    let updated = 0;
            ...
            for (let s = 0; s <= det.number_of_seasons; s++) {
                try {
                    ...
                    const episodes = sd.episodes.map(ep => {
                        ...
                        return { number: ep.episode_number, name: ep.name || `Episode ${ep.episode_number}`, air_date: ep.air_date || null, is_watched: existing?.is_watched || false, watched_at: existing?.watched_at || null, rewatch_count: existing?.rewatch_count || 0, rewatch_history: existing?.rewatch_history || [], is_special: existing?.is_special || (s === 0), my_rating: existing?.my_rating || null };
                    });
                ...
                } catch(e) { logError('Sync season', e); }
            }
            if (newSeasons.length > 0) {
                await updateDoc(doc(db, 'series', show.docId), { seasons: newSeasons, tmdb_status: det.status || show.tmdb_status, last_synced: new Date().toISOString() });
                show.seasons = newSeasons; show.tmdb_status = det.status || show.tmdb_status; updated++;
            }
            await new Promise(r => setTimeout(r, 400));
        } catch(e) { logError(`Sync ${show.title}`, e); }
    }
    localStorage.setItem('lastEpisodeSync', Date.now().toString()); saveTmdbCache();
    if (!silent && statusEl) statusEl.innerHTML = `<p style="color:var(--green);">✓ Synced ${updated}!</p>`;
    if (updated > 0) await loadMyList();
}

function findExistingEpisode(existingSeason, epNumber, tmdbName, isSeasonZero) {
    if (!existingSeason) return null; const eps = existingSeason.episodes || [];
    if (isSeasonZero) return eps.find(e => e.number === epNumber) || null;
    return eps.find(e => e.number === epNumber && !e.is_special) || eps.find(e => e.number === epNumber) || null;
}

function detectImposters(episodes, tmdbEpMap, existingSeason) {
    const byNumber = {}; episodes.forEach((ep, idx) => { if (!byNumber[ep.number]) byNumber[ep.number] = []; byNumber[ep.number].push({ ep, idx }); });
    ...
    Object.entries(byNumber).forEach(([numStr, group]) => {
        ...
        group.forEach(({ ep, idx }) => { const score = titleSimilarity(ep.name || '', officialTitle); if (score > bestScore) { bestScore = score; bestMatch = idx; } });
        group.forEach(({ ep, idx }) => {
            if (idx !== bestMatch) {
                const existingEp = existingSeason?.episodes?.find(e => e.number === ep.number && e.name === ep.name);
                result[idx] = { ...result[idx], is_special: true, ...(existingEp ? { is_watched: existingEp.is_watched, watched_at: existingEp.watched_at, rewatch_count: existingEp.rewatch_count || 0, rewatch_history: existingEp.rewatch_history || [], my_rating: existingEp.my_rating || null } : {}) };
            }
        });
    });
    return result;
...
function titleSimilarity(a, b) { if (!a || !b) return 0; const wa = a.toLowerCase().split(/\s+/); const wb = new Set(b.toLowerCase().split(/\s+/)); return wa.filter(w => wb.has(w)).length / Math.max(wa.length, wb.size); }
function titlesMatch(a, b) { return titleSimilarity(a, b) > 0.5; }

// ===== TASTE PROFILE =====
function buildTasteProfile(items) {
    const profile = { genres: {}, networks: {}, languages: {}, totalRated: 0, avgRating: 0 }; let totalScore = 0;
    items.forEach(item => { const rating = item.my_rating; if (!rating || rating < 1) return; profile.totalRated++; totalScore += rating; const weight = rating / 10;
        (item.genres || []).forEach(g => { if (g === 'Animation' && item.is_anime) return; profile.genres[g] = (profile.genres[g] || 0) + weight; });
        (item.networks || []).slice(0, 1).forEach(n => { profile.networks[n] = (profile.networks[n] || 0) + weight; });
        if (item.original_language) profile.languages[item.original_language] = (profile.languages[item.original_language] || 0) + weight;
    });
    profile.avgRating = profile.totalRated > 0 ? totalScore / profile.totalRated : 5; return profile;
}

function calculateMatchScore(showDetails, profile) {
    if (profile.totalRated < 3) return null; let score = 0, maxScore = 0;
    const showGenres = (showDetails.genres || []).map(g => typeof g === 'object' ? g.name : g);
    if (showGenres.length > 0) { maxScore += 60; const topGenres = Object.entries(profile.genres).sort((a,b) => b[1]-a[1]).slice(0, 8); const topGenreNames = new Set(topGenres.map(([g]) => g)); score += (showGenres.filter(g => topGenreNames.has(g)).length / Math.max(showGenres.length, 1)) * 60; }
    if (showDetails.original_language && Object.keys(profile.languages).length) { maxScore += 20; const topLang = Object.entries(profile.languages).sort((a,b) => b[1]-a[1])[0]?.[0]; if (showDetails.original_language === topLang) score += 20; else if (profile.languages[showDetails.original_language]) score += 10; }
    if (showDetails.vote_average) { maxScore += 20; score += Math.max(0, 20 - Math.abs(showDetails.vote_average - profile.avgRating) * 4); }
    return maxScore > 0 ? Math.round((score / maxScore) * 100) : null;
}

// ===== NAV / TABS / SEARCH =====
function setupBottomNav() { document.querySelectorAll('.nav-btn').forEach(btn => { btn.addEventListener('click', () => switchSection(btn.dataset.section)); }); }
...
function switchSubTab(tabId) {
    ...
    const page = tabEl.closest('.section-page'); if (!page) return;
...
document.querySelectorAll('.sub-tab-btn').forEach(btn => { btn.addEventListener('click', () => switchSubTab(btn.dataset.subtab)); });
...
function setupSubTabSwipe() {
    document.querySelectorAll('.swipeable-tabs').forEach(container => {
        ...
        container.addEventListener('touchend', (e) => {
            const diff = startX - e.changedTouches[0].clientX; if (Math.abs(diff) < 60) return;
            const page = container.closest('.section-page'); const tabs = page.querySelectorAll('.sub-tab-btn');
            const activeTab = page.querySelector('.sub-tab-btn.active'); const currentIdx = parseInt(activeTab?.dataset.index || '0');
            let newIdx; if (diff > 0) newIdx = Math.min(currentIdx + 1, tabs.length - 1); else newIdx = Math.max(currentIdx - 1, 0);
...
function setupSearch() {
    const topInput = document.getElementById('search-input'); const overlayInput = document.getElementById('search-overlay-input');
    const btn = document.getElementById('search-btn'); const closeBtn = document.getElementById('close-search-btn'); const clearBtn = document.getElementById('search-clear-btn');
    ...
    document.querySelectorAll('.search-filter-btn').forEach(btn => {
        btn.addEventListener('click', () => { document.querySelectorAll('.search-filter-btn').forEach(b => b.classList.remove('active')); btn.classList.add('active'); currentSearchType = btn.dataset.type === 'anime' ? 'tv' : btn.dataset.type; if (overlayInput.value.trim()) performSearch(); });
    });
}

function showSearchOverlay() { document.getElementById('search-overlay').style.display = 'block'; setTimeout(() => document.getElementById('search-overlay-input').focus(), 100); }
function hideSearchOverlay() { document.getElementById('search-overlay').style.display = 'none'; document.getElementById('search-input').blur(); }

async function performSearch() {
    const query = document.getElementById('search-overlay-input').value.trim(); if (!query) return;
    const container = document.getElementById('search-results'); container.innerHTML = '<p class="empty-state">Searching...</p>';
    try {
        ...
        if (document.querySelector('.search-filter-btn.active')?.dataset.type === 'anime') results = results.filter(r => r.media_type === 'tv' || currentSearchType === 'tv');
...
function displaySearchResults(results) {
    ...
    container.innerHTML = results.map(item => {
        const title = item.title || item.name || 'Unknown'; const year = (item.release_date || item.first_air_date || '').substring(0, 4);
        const type = item.media_type || currentSearchType; if (type === 'person') return '';
        ...
        const safeTitle = title.replace(/'/g, "\\'").replace(/"/g, '&quot;'); const safePosterUrl = poster.replace(/'/g, "\\'");
        return `<div class="media-card" onclick="openPreview(${item.id},'${type}','${safeTitle}','${year}','${safePosterUrl}')">
            <img src="${poster}" alt="${title}" onerror="this.src='${PLACEHOLDER_POSTER}'">
            <div class="info"><h3>${title}</h3><p class="year">${year} · ⭐${rating}</p></div>
            <button class="add-btn ${isInList ? 'in-list-btn' : ''}" onclick="event.stopPropagation(); ${isInList ? `removeFromListByTMDB(${item.id},'${type}')` : `addToList(${item.id},'${type}','${safeTitle}','${year}','${safePosterUrl}')`}">${isInList ? '✓ In Library' : '+ Add'}</button>
        </div>`;
    }).filter(Boolean).join('');
}
...
async function loadMyList() {
    ...
    try {
        myList = [];
        const [moviesSnap, seriesSnap] = await Promise.all([getDocs(collection(db, 'movies')), getDocs(collection(db, 'series'))]);
        ...
        autoTagStatusesSilent(); renderAllSections(); updateProfilePage();
...
function getAiredEpisodesOnly(seasons) {
    ...
    const aired = [];
    (seasons || []).forEach(s => { if (s.number === 0) return; (s.episodes || []).forEach(ep => { if (ep.is_special) return; const airDate = ep.air_date ? new Date(ep.air_date) : null; if (!airDate || airDate <= today) aired.push({ ...ep, seasonNum: s.number }); }); });
    return aired;
}
...
function getReWatchProgress(show) { const aired = getAiredEpisodesOnly(show.seasons); if (!aired.length) return 0; const maxRewatch = Math.max(...aired.map(ep => ep.rewatch_count || 0)); if (maxRewatch === 0) return 0; return (aired.filter(ep => (ep.rewatch_count || 0) >= maxRewatch).length / aired.length) * 100; }

function getNextEpisodeExcludingSpecials(show) {
    const today = new Date(); today.setHours(23, 59, 59, 999);
    if (!show.seasons) return null;
    for (const s of show.seasons) { if (s.number === 0) continue; for (const ep of (s.episodes || [])) { if (ep.is_special) continue; const airDate = ep.air_date ? new Date(ep.air_date) : null;
        // If show allows marking unaired, don't skip unaired
        if (airDate && airDate > today && !show.allow_mark_unaired) continue;
...
function getNextReWatchEpisode(show) {
    ...
    for (const s of show.seasons) { if (s.number === 0) continue; for (const ep of (s.episodes || [])) { if (ep.is_special) continue; const airDate = ep.air_date ? new Date(ep.air_date) : null; if (airDate && airDate > today) continue; if ((ep.rewatch_count || 0) < targetCount) return { season: s.number, number: ep.number, name: ep.name || `Episode ${ep.number}` }; }}
    return null;
}
...
function getMostRecentAirDate(show) { const today = new Date(); let mostRecent = null; (show.seasons || []).forEach(s => { if (s.number === 0) return; (s.episodes || []).forEach(ep => { if (!ep.air_date || ep.is_special) return; const d = new Date(ep.air_date); if (d <= today && (!mostRecent || d > mostRecent)) mostRecent = d; }); }); return mostRecent; }

function getAllWatchedEpisodes(shows) {
    const eps = []; shows.forEach(show => { show.seasons?.forEach(s => { if (s.number === 0) return; s.episodes?.forEach(ep => { if (ep.is_watched && ep.watched_at) eps.push({ show: show.title, poster: show.poster, docId: show.docId, season: s.number, episode: ep.number, name: ep.name, watched_at: ep.watched_at, is_special: ep.is_special || false }); }); }); });
    eps.sort((a, b) => new Date(b.watched_at) - new Date(a.watched_at)); return eps;
}

function getPreviousUnwatchedEpisodes(show, targetSeason, targetEp) {
    const today = new Date(); today.setHours(23, 59, 59, 999); const unwatched = [];
    if (!show.seasons) return unwatched;
    for (const s of show.seasons) { if (s.number === 0) continue; if (s.number > targetSeason) break; for (const ep of (s.episodes || [])) { if (ep.is_special) continue; if (s.number === targetSeason && ep.number >= targetEp) break; const airDate = ep.air_date ? new Date(ep.air_date) : null; if (airDate && airDate > today && !show.allow_mark_unaired) continue; if (!ep.is_watched) unwatched.push({ seasonNum: s.number, episodeNum: ep.number }); }}
    return unwatched;
}

function getEpisodesNeedingRewatch(show, targetSeason, targetEp) {
    const needsRewatch = []; if (!show.seasons) return needsRewatch;
    ...
    for (const s of show.seasons) { if (s.number === 0) continue; if (s.number > targetSeason) break; for (const ep of (s.episodes || [])) { if (ep.is_special) continue; if (s.number === targetSeason && ep.number >= targetEp) break; if (!ep.is_watched) continue; if ((ep.rewatch_count || 0) < targetRewatchCount) needsRewatch.push({ seasonNum: s.number, episodeNum: ep.number }); }}
    return needsRewatch;
}

function isAnimeShow(details) { const genres = details.genres || []; const isAnimation = genres.some(g => g.id === 16); const isJapanese = details.original_language === 'ja'; const isChinese = details.original_language === 'zh'; const animeNets = ['Fuji TV','Tokyo MX','TBS','TV Tokyo','Crunchyroll','AT-X','BS11','MBS','NHK','Bilibili']; return (isAnimation && (isJapanese || isChinese)) || (details.networks || []).some(n => animeNets.includes(n.name)); }

function formatWatchTime(totalMinutes) { const years = Math.floor(totalMinutes / 525600); const months = Math.floor((totalMinutes % 525600) / 43800); const days = Math.floor((totalMinutes % 43800) / 1440); const hours = Math.floor((totalMinutes % 1440) / 60); let parts = []; if (years > 0) parts.push(`${years}y`); if (months > 0) parts.push(`${months}m`); if (days > 0) parts.push(`${days}d`); parts.push(`${hours}h`); return parts.join(' '); }

function getTimelineLabel(dateStr) { const date = new Date(dateStr); const now = new Date(); const today = new Date(now.getFullYear(), now.getMonth(), now.getDate()); const weekAgo = new Date(today); weekAgo.setDate(weekAgo.getDate()-7); const twoWeeksAgo = new Date(today); twoWeeksAgo.setDate(twoWeeksAgo.getDate()-14); if (date >= today) return 'Today'; if (date >= weekAgo) return 'This Week'; if (date >= twoWeeksAgo) return 'Last Week'; return date.toLocaleDateString('en-US', { month: 'long', year: 'numeric' }); }

function autoTagStatusesSilent() {
    myList.forEach(item => { if (item.type !== 'tv') return; if (item.user_status === 'Rewatching') return;
...
function renderJumpPills(sectionType, sections) {
    ...
    if (activeSections.length <= 1) { container.innerHTML = ''; return; }
    container.innerHTML = `<div class="section-jump-pills">${activeSections.map(s => `
        <button class="section-pill" data-target="${s.id}" onclick="jumpToSection('${sectionType}','${s.id}')">${s.icon} ${s.label} (${s.count})</button>`).join('')}</div>`;
    setupScrollSpy(sectionType, activeSections);
}

function jumpToSection(sectionType, targetId) {
    const el = document.getElementById(targetId); if (!el) return;
    const topBarH = document.querySelector('.top-bar')?.offsetHeight || 60;
    const subTabH = 40;
    const pillsH  = document.querySelector(`#${sectionType}-jump-pills .section-jump-pills`)?.offsetHeight || 40;
    window.scrollTo({ top: el.offsetTop - topBarH - subTabH - pillsH - 10, behavior: 'smooth' });
    document.querySelectorAll(`#${sectionType}-jump-pills .section-pill`).forEach(p => p.classList.toggle('active', p.dataset.target === targetId));
}

function setupScrollSpy(sectionType, sections) {
    let scrollTimer;
    const handler = () => { clearTimeout(scrollTimer); scrollTimer = setTimeout(() => {
        ...
        document.querySelectorAll(`#${sectionType}-jump-pills .section-pill`).forEach(p => p.classList.toggle('active', p.dataset.target === activeId));
    }, 100); };
...
function renderContinueWatching(sectionType) {
    ...
    const inProgress = shows.filter(item => {
        if (item.user_status === 'Rewatching' || item.user_status === 'Dropped' || item.user_status === 'Planned') return false;
        ...
        item.seasons?.forEach(s => { if (s.number === 0) return; s.episodes?.forEach(ep => {
            ...
            if (airDate && airDate > today && !item.allow_mark_unaired) return;
            if (ep.is_watched) hasWatched = true; else hasUnwatched = true;
        }); });
    ...
    const continueWatching   = inProgress.filter(s => { if (s.user_status === 'Paused' || isCurrentlyAiring(s)) return false; return new Date(getLastWatchedDate(s)) >= sixtyDaysAgo; });
    const notWatchedInAWhile = inProgress.filter(s => { if (s.user_status === 'Paused' || isCurrentlyAiring(s)) return false; return new Date(getLastWatchedDate(s)) < sixtyDaysAgo; });
    ...
    let html = '';
    if (currentlyAiring.length) { html += `<div class="continue-section-label" id="${sectionType}-sec-airing">📡 Currently Airing</div>`; html += currentlyAiring.map(s => createContinueCard(s)).join(''); }
    if (continueWatching.length) { html += `<div class="continue-section-label" id="${sectionType}-sec-continue">▶ Continue Watching</div>`; html += continueWatching.map(s => createContinueCard(s)).join(''); }
    if (rewatching.length) { html += `<div class="continue-section-label" id="${sectionType}-sec-rewatch">↺ Rewatching</div>`; html += rewatching.map(s => createContinueCard(s, false, true)).join(''); }
    if (notWatchedInAWhile.length) { html += `<div class="continue-section-label" id="${sectionType}-sec-stale">💤 Haven't Watched in a While</div>`; html += notWatchedInAWhile.map(s => createContinueCard(s)).join(''); }
    if (notStarted.length) { html += `<div class="continue-section-label" id="${sectionType}-sec-notstarted">📋 Haven't Started</div>`; html += notStarted.map(s => createContinueCard(s)).join(''); }
    if (paused.length) { html += `<div class="continue-section-label" id="${sectionType}-sec-paused">⏸ Paused</div>`; html += paused.map(s => createContinueCard(s, true)).join(''); }
...
function createContinueCard(show, forcefade = false, isRewatching = false) {
    ...
    const epCode    = nextEp ? `S${String(nextEp.season).padStart(2,'0')}E${String(nextEp.number).padStart(2,'0')}` : isRewatchMode ? 'Rewatch complete' : 'Up to date';
    const isPaused = show.user_status === 'Paused';
    const airing   = isCurrentlyAiring(show);

    let progressBarHTML;
    if (isRewatchMode) {
        progressBarHTML = `<div class="continue-progress rewatch-bar"><div class="continue-progress-fill" style="width:${progress}%;"></div></div>`;
    } else {
        progressBarHTML = `<div class="continue-progress"><div class="continue-progress-fill ${progress >= 100 ? 'uptodate' : 'watching'}" style="width:${progress}%;"></div></div>`;
    }
    ...
    return `
        <div class="continue-card ${(isPaused || forcefade) ? 'paused-card' : ''}">
            <img src="${poster}" alt="${show.title}" onerror="this.src='${PLACEHOLDER_THUMB}'" onclick="openDetails('${safeDocId}','tv')">
            <div class="continue-info">
                <h3 onclick="openDetails('${safeDocId}','tv')">${show.title}</h3>
                <div class="episode-code">${isPaused?'⏸ ':''}${airing?'🟢 ':''}${isRewatchMode?'↺ ':''}${epCode}${remainingHTML}</div>
                ${nextEp ? `<div class="episode-name">${nextEp.name}</div>` : ''}
                ${progressBarHTML}
            </div>
            ${nextEp && !isPaused ? `<button class="quick-check-btn" onclick="quickMarkWatched('${safeDocId}',${nextEp.season},${nextEp.number},${isRewatchMode})">✓</button>` : '<div style="width:40px;"></div>'}
        </div>`;
}
...
function renderHistory(sectionType) {
    ...
    Object.entries(groups).forEach(([label, eps]) => {
        ...
        html += eps.map(ep => {
            const poster = safePoster(ep.poster, 'small'); const safeDocId = ep.docId.replace(/'/g, "\\'");
            const epCode = ep.is_special ? 'Special' : `S${String(ep.season).padStart(2,'0')}E${String(ep.episode).padStart(2,'0')}`;
            return `<div class="history-card"><img src="${poster}" onerror="this.src='${PLACEHOLDER_SMALL}'" onclick="openDetails('${safeDocId}','tv')"><div class="history-info"><h4 onclick="openDetails('${safeDocId}','tv')">${ep.show}</h4><div class="history-ep">${epCode} - ${ep.name||'Episode'}</div><div class="history-date">${new Date(ep.watched_at).toLocaleDateString()}</div></div></div>`;
        }).join('');
    });
    container.innerHTML = html;
}
...
function renderMoviesSection() {
    ...
    const watchedEl = document.getElementById('movies-watched-list'); const unwatchedEl = document.getElementById('movies-unwatched-list');
    const watched = movies.filter(m => m.is_watched); const unwatched = movies.filter(m => !m.is_watched);
...
function createMediaCard(item) {
    const poster = safePoster(item.poster); let statusLine = '';
    ...
    return `<div class="media-card" onclick="openDetails('${safeDocId}','${item.type}')">
        ...
        <div class="info"><h3>${item.title||'Unknown'}</h3><p class="year">${rating||item.year||''}</p></div></div>`;
}

// ===== PROFILE =====
function updateProfilePage() {
    const anime=getAnime(), tv=getTVShows(), movies=getMovies();
    function countEps(list) { let t=0; list.forEach(s => s.seasons?.forEach(season => { if (season.number===0) return; season.episodes?.forEach(ep => { if (ep.is_watched && !ep.is_special) t++; }); })); return t; }
...
async function quickMarkWatched(docId, seasonNum, episodeNum, isRewatchMode = false) {
    ...
    const episode=season.episodes.find(e=>e.number===episodeNum&&!e.is_special); if(!episode) return;
    ...
    if (isRewatchMode) {
        ...
        if (prev.length>0) {
            ...
            if(a==='yes'){
                ...
                prev.forEach(({seasonNum:sN,episodeNum:eN},idx)=>{const s=item.seasons.find(s=>s.number===sN);const e=s?.episodes.find(e=>e.number===eN&&!e.is_special);if(e){e.is_watched=true;e.watched_at=timestamps[idx];}});
                episode.is_watched=true;episode.watched_at=timestamps[timestamps.length-1];
            } else if(a==='no'){episode.is_watched=true;episode.watched_at=new Date().toISOString();}
            else return;
        } else {episode.is_watched=true;episode.watched_at=new Date().toISOString();}
    }
    try{await updateDoc(doc(db,'series',docId),{seasons:item.seasons});const section=item.is_anime?'anime':'tv';renderContinueWatching(section);renderHistory(section);}catch(e){logError('Quick mark',e);}
}
...
function showConfirm(title, message, yesText='Yes', noText='No', showCancel=false) {
    return new Promise((resolve) => {
        ...
        const yesBtn=document.getElementById('confirm-yes');const noBtn=document.getElementById('confirm-no');
        const cancelBtn=document.getElementById('confirm-cancel');const closeBtn=dialog.querySelector('.confirm-close');
        yesBtn.textContent=yesText;noBtn.textContent=noText;cancelBtn.style.display=showCancel?'inline-block':'none';
        ...
        const cleanup=()=>{closeModal('confirm-dialog');yesBtn.replaceWith(yesBtn.cloneNode(true));noBtn.replaceWith(noBtn.cloneNode(true));cancelBtn.replaceWith(cancelBtn.cloneNode(true));if(closeBtn)closeBtn.replaceWith(closeBtn.cloneNode(true));};
...
function showRewatchConfirm(episodeName) {
    return new Promise((resolve) => {
        ...
        cancelBtn.className='confirm-btn confirm-cancel-btn';cancelBtn.style.cssText='';
        openModal('confirm-dialog');
        const cleanup=()=>{closeModal('confirm-dialog');yesBtn.style.cssText='';noBtn.style.cssText='';yesBtn.replaceWith(yesBtn.cloneNode(true));noBtn.replaceWith(noBtn.cloneNode(true));cancelBtn.replaceWith(cancelBtn.cloneNode(true));if(closeBtn)closeBtn.replaceWith(closeBtn.cloneNode(true));};
...
function showMarkPreviousConfirm(count) {
    return new Promise((resolve) => {
        ...
        const yesBtn=document.getElementById('confirm-yes');const noBtn=document.getElementById('confirm-no');
        const cancelBtn=document.getElementById('confirm-cancel');const closeBtn=dialog.querySelector('.confirm-close');
        yesBtn.textContent='Yes, all';noBtn.textContent='Just this';cancelBtn.style.display='none';
        yesBtn.className='confirm-btn confirm-yes';yesBtn.style.cssText='';
        noBtn.className='confirm-btn';noBtn.style.cssText='background:var(--blue);color:white;';
        openModal('confirm-dialog');
        const cleanup=()=>{closeModal('confirm-dialog');noBtn.style.cssText='';yesBtn.replaceWith(yesBtn.cloneNode(true));noBtn.replaceWith(noBtn.cloneNode(true));cancelBtn.replaceWith(cancelBtn.cloneNode(true));if(closeBtn)closeBtn.replaceWith(closeBtn.cloneNode(true));};
        document.getElementById('confirm-yes').addEventListener('click',()=>{cleanup();resolve('yes');});
        document.getElementById('confirm-no').addEventListener('click',()=>{cleanup();resolve('no');});
        dialog.querySelector('.confirm-close')?.addEventListener('click',()=>{cleanup();resolve('cancel');});
    });
...
async function openPreview(tmdbId, type, title, year, poster) {
    ...
    const similarItems=similar?.results?.slice(0,10)||[];const providerList=providers?.results?.US?.flatrate||[];
    ...
    const similarWithScores=similarItems.map(item=>({...item,matchScore:calculateMatchScore(item,tasteProfile)})).sort((a,b)=>(b.matchScore||0)-(a.matchScore||0));
    ...
    if(type==='tv'&&details?.number_of_seasons){
        contentHTML=`<div class="detail-tabs" style="margin-top:20px;">
            ...
            <div class="detail-tab-content active" id="preview-info-tab">
                ...
                ${buildCastSection(cast)}${buildNetworksSection(providerList,networks)}${buildSimilarSection(similarWithScores,type)}</div>
...
function buildMyRatingWidget(item, safeDocId) {
    const current=item.my_rating||0; const col=item.type==='movie'?'movies':'series';
...
async function openMovieDetails(item, body, safeDocId) {
    let details,credits,similar,providers;
    if(item.tmdb_id){try{[details,credits,similar,providers]=await Promise.all([
    ...
    const synopsis=details?.overview||'No synopsis.';const rating=details?.vote_average||item.tmdb_rating;
    const genres=details?.genres||[];const runtime=details?.runtime?`${Math.floor(details.runtime/60)}h ${details.runtime%60}m`:'N/A';
    const cast=credits?.cast?.slice(0,15)||[];const director=credits?.crew?.find(c=>c.job==='Director');
    const similarItems=similar?.results?.slice(0,10)||[];const providerList=providers?.results?.US?.flatrate||[];

    const allItems=[...getAnime(),...getTVShows(),...getMovies()];
    const tasteProfile=buildTasteProfile(allItems);
    const similarWithScores=similarItems.map(si=>({...si,matchScore:calculateMatchScore(si,tasteProfile)})).sort((a,b)=>(b.matchScore||0)-(a.matchScore||0));
...
async function openTVDetails(item, body, safeDocId) {
    ...
    ]);}catch(e){logError('TV detail',e);}}

    const synopsis=details?.overview||'No synopsis.';const rating=details?.vote_average||item.tmdb_rating;
    const genres=details?.genres||[];const cast=credits?.cast?.slice(0,15)||[];
    const similarItems=similar?.results?.slice(0,10)||[];const providerList=providers?.results?.US?.flatrate||[];
    const networks=details?.networks||[];const tmdbStatus=details?.status||item.tmdb_status||'Unknown';
    ...
    const specialsHTML=allSpecialEps.length?`<div class="season specials">
        ...
        <div class="detail-header-info">
            <h2><span>${item.title}</span>
                ...
                <div class="options-menu" id="t-opts">
                    ...
                    <button onclick="toggleAllowMarkUnaired('${safeDocId}')">${item.allow_mark_unaired?'📅 Disable Mark Unaired':'📅 Allow Mark Unaired'}</button>
            ...
            <div>
                ...
                ${item.allow_mark_unaired?'<span class="status-badge" style="background:#FF9800;font-size:9px;">Mark Unaired</span>':''}
            </div>
...
async function toggleHideFromList(docId, type) {
    ...
    try {
        ...
        await updateDoc(doc(db, col, docId), { hide_from_list: item.hide_from_list });
        // Re-render detail page to update menu text
        const body = document.getElementById('modal-body');
        if (body && document.getElementById('modal').style.display !== 'none') {
            if (type === 'movie') await openMovieDetails(item, body, docId.replace(/'/g, "\\'"));
            else await openTVDetails(item, body, docId.replace(/'/g, "\\'"));
        }
    } catch(e) { logError('Toggle hide', e); }
}

// ===== TOGGLE ALLOW MARK UNAIRED =====
async function toggleAllowMarkUnaired(docId) {
    const item=myList.find(i=>i.docId===docId); if(!item) return;
    item.allow_mark_unaired = !item.allow_mark_unaired;
    try {
        await updateDoc(doc(db, 'series', docId), { allow_mark_unaired: item.allow_mark_unaired });
        const body = document.getElementById('modal-body');
        if (body && document.getElementById('modal').style.display !== 'none') {
            await openTVDetails(item, body, docId.replace(/'/g, "\\'"));
        }
    } catch(e) { logError('Toggle unaired', e); }
}

// ===== SEASON / EPISODE BUILD =====
function toggleSeason(header, docId, seasonNum) {
    const body=header.nextElementSibling; const icon=header.querySelector('.toggle-icon');
...
function buildSeasonHTML(season, safeDocId, docId, allowMarkUnaired) {
    ...
    return `<div class="season">
        ...
        <div class="season-body ${isExpanded?'open':''}">
            ${regularEps.map(ep=>buildEpisodeHTML(ep,season.number,safeDocId,allowMarkUnaired)).join('')||'<p style="padding:10px;color:var(--text3);">No episodes</p>'}
...
function buildEpisodeHTML(ep, seasonNum, safeDocId, allowMarkUnaired) {
    const today=new Date(); today.setHours(23,59,59,999);
    const airDate=ep.air_date?new Date(ep.air_date):null;
    const isUnaired=airDate&&airDate>today;
    const canMark = !isUnaired || allowMarkUnaired;
...
// ===== BUILD HELPERS =====
function buildCastSection(cast) { if(!cast.length) return ''; return `<div class="cast-section"><h3>🎭 Cast</h3><div class="cast-carousel" onwheel="event.preventDefault();this.scrollLeft+=event.deltaY;">${cast.map(p=>`<div class="cast-card"><img src="${p.profile_path?TMDB_IMG_BASE+p.profile_path:PLACEHOLDER_AVATAR}" alt="${p.name}" onerror="this.src='${PLACEHOLDER_AVATAR}'"><div class="cast-name">${p.name}</div><div class="cast-character">${p.character||''}</div></div>`).join('')}</div></div>`; }

function buildNetworksSection(providers, networks) { const all=[...(networks||[]),...(providers||[])]; if(!all.length) return ''; return `<div class="networks-section"><h3>📺 Available On</h3><div class="network-logos">${all.map(n=>n.logo_path?`<img class="network-logo" src="${TMDB_IMG_BASE}${n.logo_path}" alt="${n.name||n.provider_name}">`:`<span class="network-name">${n.name||n.provider_name}</span>`).join('')}</div></div>`; }
...
async function openEpisodeDetail(docId, seasonNum, episodeNum, isSpecial=false, epName='') {
    const item=myList.find(i=>i.docId===docId); if(!item?.tmdb_id) return;
    ...
    try{
        ...
        const sd=docId.replace(/'/g,"\\'"); const safeEpName=(epName||'').replace(/'/g,"\\'");
        ...
        epBody.innerHTML=`<div class="ep-detail-header">
            ...
            <div style="margin:15px 0;">
                <button onclick="toggleEpisode('${sd}',${seasonNum},${episodeNum},${isSpecial},'${safeEpName}');closeModal('episode-modal');" class="watch-btn ${localEp?.is_watched?'watched':'mark-watched'}" style="padding:10px 24px;">${localEp?.is_watched?'✓ Watched':'○ Mark Watched'}</button>
    ...
    }catch(e){epBody.innerHTML='<p class="empty-state">Failed to load.</p>';logError('Episode detail',e);}
}
    ...
    if(episode.is_watched){
        ...
        if(choice==='from-start'){
            ...
            needsRewatch.forEach(({seasonNum:sN,episodeNum:eN},idx)=>{
                const s=item.seasons.find(s=>s.number===sN);const e=s?.episodes.find(e=>e.number===eN&&!e.is_special);
            ...
            episode.rewatch_count=(episode.rewatch_count||0)+1;if(!episode.rewatch_history)episode.rewatch_history=[];
            episode.rewatch_history.push(timestamps[timestamps.length-1]);episode.watched_at=timestamps[timestamps.length-1];
        }else if(choice==='just-this'){
            episode.rewatch_count=(episode.rewatch_count||0)+1;if(!episode.rewatch_history)episode.rewatch_history=[];
            episode.rewatch_history.push(new Date().toISOString());episode.watched_at=new Date().toISOString();
        }else if(choice==='unmark'){episode.is_watched=false;episode.watched_at=null;}
        else return;
    } else {
...
async function markSeasonWatched(docId, seasonNum) {
    ...
    const regularEps=season.episodes.filter(ep=>{
        ...
        const airDate=ep.air_date?new Date(ep.air_date):null;
        // If allow_mark_unaired, include unaired eps
        if(airDate&&airDate>today&&!item.allow_mark_unaired)return false;
        return true;
    });
    ...
    if(allWatched){
        ...
        if(a==='yes'){const timestamps=generateIncrementalTimestamps(regularEps.length,item.is_anime);regularEps.forEach((ep,idx)=>{ep.rewatch_count=(ep.rewatch_count||0)+1;if(!ep.rewatch_history)ep.rewatch_history=[];ep.rewatch_history.push(timestamps[idx]);ep.watched_at=timestamps[idx];});}
        ...
        if(seasonNum!==0){
            ...
            if(prevSeasons.length>0){
                ...
                if(a==='yes'){
                    const allPrevEps=[];prevSeasons.forEach(s=>s.episodes.filter(ep=>!ep.is_special&&!ep.is_watched).forEach(ep=>{allPrevEps.push({s,ep});}));
                    const unwatchedCurrent=regularEps.filter(ep=>!ep.is_watched);
                    const total=allPrevEps.length+unwatchedCurrent.length;const timestamps=generateIncrementalTimestamps(total,item.is_anime);
                    let tsIdx=0;allPrevEps.forEach(({ep})=>{ep.is_watched=true;ep.watched_at=timestamps[tsIdx++];});
                    unwatchedCurrent.forEach(ep=>{ep.is_watched=true;ep.watched_at=timestamps[tsIdx++];});
                }else{const unwatched=regularEps.filter(ep=>!ep.is_watched);const timestamps=generateIncrementalTimestamps(unwatched.length,item.is_anime);unwatched.forEach((ep,idx)=>{ep.is_watched=true;ep.watched_at=timestamps[idx];});}
            }else{const unwatched=regularEps.filter(ep=>!ep.is_watched);const timestamps=generateIncrementalTimestamps(unwatched.length,item.is_anime);unwatched.forEach((ep,idx)=>{ep.is_watched=true;ep.watched_at=timestamps[idx];});}
        }else{const unwatched=regularEps.filter(ep=>!ep.is_watched);const timestamps=generateIncrementalTimestamps(unwatched.length,item.is_anime);unwatched.forEach((ep,idx)=>{ep.is_watched=true;ep.watched_at=timestamps[idx];});}
    }
    ...
    try{
        ...
        const section=item.is_anime?'anime':'tv';renderContinueWatching(section);renderHistory(section);
...
async function toggleWatched(docId,type){
    const item=myList.find(i=>i.docId===docId);if(!item)return;
    if(item.is_watched){const a=await showConfirm('Already Watched','What to do?','↺ Rewatch','✗ Unmark');
    ...
    else if(a==='no'){item.is_watched=false;item.watched_at=null;}else return;}
...
async function setUserStatus(docId,status){try{await updateDoc(doc(db,'series',docId),{user_status:status});const item=myList.find(i=>i.docId===docId);if(item)item.user_status=status;await loadMyList();openDetails(docId,'tv');}catch(e){logError('Status',e);}}
...
async function addToList(tmdbId,type,title,year,poster){
    try{
        ...
        if(type==='tv'){
            ...
            // Fetch air time for new show
            setTimeout(()=>{const show=myList.find(i=>i.docId===docId);if(show)fetchAirTimeData(show);},2000);
        }else{
    ...
    }catch(e){logError('Add to list',e);alert('Error adding.');}
}

async function removeFromList(docId,type){
    const a=await showConfirm('Remove?','Remove from library?','Remove','Cancel');
    if(a!=='yes')return;
    try{await deleteDoc(doc(db,type==='movie'?'movies':'series',docId));await loadMyList();closeModal('modal');}catch(e){logError('Remove',e);}
}
...
function openTagSpecialsModal(docId) {
    ...
    if(!modal){modal=document.createElement('div');modal.id='tag-specials-modal';modal.className='modal';modal.style.zIndex='3000';
    modal.innerHTML=`<div class="modal-content" style="max-width:620px;"><span class="close" onclick="closeModal('tag-specials-modal')">&times;</span><div id="tag-specials-body"></div></div>`;document.body.appendChild(modal);}
...
function buildTagSpecialsList(item, filterSeason) {
    ...
    (s.episodes||[]).forEach(ep=>{const isSpecial=ep.is_special||s.number===0;
    ...
    rows+=`<div style="display:flex;align-items:center;gap:10px;padding:8px 4px;border-bottom:1px solid var(--border);">
        ...
        <label style="font-size:13px;color:var(--text);cursor:pointer;flex:1;">${label}${isSpecial?'<span style="background:#FF6B35;color:white;padding:1px 6px;border-radius:8px;font-size:10px;margin-left:6px;">SPECIAL</span>':''}</label></div>`;});});
...
async function applySpecialTags(docId){
    const item=myList.find(i=>i.docId===docId);if(!item)return;
    document.querySelectorAll('.tag-special-cb').forEach(cb=>{const seasonNum=parseInt(cb.dataset.season);const epNum=parseInt(cb.dataset.ep);const epName=cb.dataset.name||'';
    const season=item.seasons.find(s=>s.number===seasonNum);if(!season)return;
    ...
    try{await updateDoc(doc(db,'series',docId),{seasons:item.seasons});closeModal('tag-specials-modal');await loadMyList();openDetails(docId,'tv',activeDetailTab);}catch(e){logError('Special tags',e);}
}

// ===== RATE YOUR SHOWS MODAL =====
function openRateShowsModal(){
    let modal=document.getElementById('rate-shows-modal');
    if(!modal){modal=document.createElement('div');modal.id='rate-shows-modal';modal.className='modal';modal.style.zIndex='3000';
    modal.innerHTML=`<div class="modal-content" style="max-width:700px;"><span class="close" onclick="closeModal('rate-shows-modal')">&times;</span><div id="rate-shows-body"></div></div>`;document.body.appendChild(modal);}
    const body=document.getElementById('rate-shows-body');
    const all=[...getAnime(),...getTVShows(),...getMovies()];
    body.innerHTML=`<h3 style="color:var(--accent);margin-bottom:8px;">⭐ Rate Your Shows</h3>
        ...
        <div style="margin-bottom:12px;display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
            ...
            <select id="rate-status-filter" onchange="filterRateList()" style="padding:6px 10px;border:2px solid var(--border);border-radius:8px;background:var(--surface);color:var(--text);font-size:13px;">
                <option value="all">All</option><option value="watched">Watched</option><option value="unrated">Unrated</option></select>
...
function buildRateShowsList(items, typeFilter, statusFilter){
    ...
    return filtered.map(item=>{
        ...
        return `<div class="rate-show-item"><img src="${poster}" onerror="this.src='${PLACEHOLDER_THUMB}'">
            <div class="rate-show-info"><div class="rate-show-title">${item.title}</div>
            <div class="rate-show-meta">${item.is_anime?'Anime':item.type==='movie'?'Movie':'TV'} · ${item.user_status||(item.is_watched?'Watched':'—')}</div></div>
...
async function rateShowInline(docId,col,rating){
    ...
    if(container){const current=rating||0;const sd=docId.replace(/'/g,"\\'");
...
async function loadSectionCalendar(section) {
    ...
    todayEl.innerHTML=`<p class="empty-state">Checking ${toCheck.length} shows...</p>`;weekEl.innerHTML='';upcomingEl.innerHTML='';
    const tEps=[],wEps=[],uEps=[];let checked=0;

    for(const show of toCheck){
        try{
            checked++;todayEl.innerHTML=`<p class="empty-state">Checking ${checked}/${toCheck.length}...</p>`;
            const det=await tmdbFetch(`${TMDB_BASE_URL}/tv/${show.tmdb_id}?api_key=${TMDB_API_KEY}`);
            ...
            if(['Returning Series','In Production'].includes(det.status)&&det.next_episode_to_air){
                ...
                // Get air time
                const airTimeData = show.air_time_data?.source !== 'default' ? show.air_time_data : null;
                let ghanaTimeStr = null;
                if (airTimeData?.time) {
                    ghanaTimeStr = convertToGhanaTime(airTimeData.time, airTimeData.timezone);
                }

                const ep={show:show.title,poster:show.poster,docId:show.docId,
    ...
    displayCalItems(todayEl,tEps,true);displayCalItems(weekEl,wEps,false);displayCalItems(upcomingEl,uEps,false);
}
...
function formatAirDate(date){const t=new Date();t.setHours(0,0,0,0);const tm=new Date(t);tm.setDate(tm.getDate()+1);const c=new Date(date);c.setHours(0,0,0,0);if(c.getTime()===t.getTime())return'Today';if(c.getTime()===tm.getTime())return'Tomorrow';return date.toLocaleDateString('en-US',{weekday:'short',month:'short',day:'numeric'});}

// ===== RESCAN / AUTO TAG =====
async function rescanAnime(){const statusEl=document.getElementById('settings-action-status');const shows=myList.filter(i=>i.type==='tv'&&i.tmdb_id);statusEl.innerHTML=`<p style="color:var(--accent);">Scanning ${shows.length}...</p>`;let changed=0;for(let i=0;i<shows.length;i++){try{statusEl.innerHTML=`<p style="color:var(--accent);">Scanning ${i+1}/${shows.length}...</p>`;const det=await tmdbFetch(`${TMDB_BASE_URL}/tv/${shows[i].tmdb_id}?api_key=${TMDB_API_KEY}`);const shouldBe=isAnimeShow(det);if(shouldBe!==shows[i].is_anime){await updateDoc(doc(db,'series',shows[i].docId),{is_anime:shouldBe});shows[i].is_anime=shouldBe;changed++;}if(i%20===0)await new Promise(r=>setTimeout(r,500));}catch(e){logError('Rescan',e);}}statusEl.innerHTML=`<p style="color:var(--green);">✓ ${changed} updated.</p>`;await loadMyList();}

async function autoTagStatuses(){const statusEl=document.getElementById('settings-action-status');statusEl.innerHTML='<p style="color:var(--accent);">Tagging...</p>';let changed=0;for(const item of myList){if(item.type!=='tv')continue;if(item.user_status==='Rewatching')continue;const progress=getShowProgressExcludingSpecials(item);const hasWatched=item.seasons?.some(s=>s.number!==0&&s.episodes?.some(e=>e.is_watched&&!e.is_special));const tmdb=item.tmdb_status||'';let newStatus=item.user_status;if(!hasWatched&&!['Dropped','Paused'].includes(item.user_status))newStatus='Planned';else if(progress>=100&&(tmdb==='Ended'||tmdb==='Canceled'))newStatus='Finished';else if(progress>=100&&tmdb==='Returning Series')newStatus='Up to Date';else if(hasWatched&&progress<100&&!['Dropped','Paused','Finished'].includes(item.user_status))newStatus='Watching';if(newStatus!==item.user_status){await updateDoc(doc(db,'series',item.docId),{user_status:newStatus});item.user_status=newStatus;changed++;}}statusEl.innerHTML=`<p style="color:var(--green);">✓ ${changed} updated.</p>`;await loadMyList();}

// ===== BULK TAGGER =====
function openBulkTagger(){const modal=document.getElementById('bulk-modal');const body=document.getElementById('bulk-modal-body');const shows=myList.filter(i=>i.type==='tv');const movies=myList.filter(i=>i.type==='movie');
body.innerHTML=`<h2 style="margin-bottom:15px;color:var(--accent);">📋 Bulk Tag</h2>
    <div style="margin-bottom:10px;display:flex;gap:8px;flex-wrap:wrap;">
        <select id="bulk-type-filter" onchange="filterBulkList()"><option value="all">All</option><option value="anime">Anime</option><option value="tv">TV</option><option value="movie">Movies</option></select>
    ...
    <div class="bulk-list" id="bulk-list">${[...shows,...movies].map(item=>`<div class="bulk-item" data-type="${item.type}" data-anime="${item.is_anime||false}">
        ...
        <span class="bulk-item-status">${item.user_status||(item.is_watched?'Watched':'—')}</span></div>`).join('')}</div>
    <div class="bulk-actions">
        <select id="bulk-action"><option value="">Choose...</option><option value="Watching">Watching</option><option value="Up to Date">Up to Date</option><option value="Rewatching">Rewatching</option><option value="Finished">Finished</option><option value="Paused">Paused</option><option value="Dropped">Dropped</option><option value="Planned">Planned</option><option value="anime-true">Tag Anime</option><option value="anime-false">Remove Anime</option><option value="fav-true">Favorite</option><option value="fav-false">Unfavorite</option></select>
        <button onclick="applyBulkAction()">Apply</button></div>`;
...
async function applyBulkAction(){const action=document.getElementById('bulk-action').value;if(!action)return;const checked=document.querySelectorAll('.bulk-item input:checked');if(!checked.length){alert('Select items first!');return;}for(const cb of checked){const docId=cb.value,itemType=cb.dataset.itemType;const col=itemType==='movie'?'movies':'series';try{if(action.startsWith('anime-'))await updateDoc(doc(db,col,docId),{is_anime:action==='anime-true'});else if(action.startsWith('fav-'))await updateDoc(doc(db,col,docId),{is_favorite:action==='fav-true'});else await updateDoc(doc(db,col,docId),{user_status:action});}catch(e){logError('Bulk action',e);}}closeModal('bulk-modal');await loadMyList();}

// ===== STATS (same as previous — abbreviated for space) =====
function openStatsPage(section){const body=document.getElementById('stats-modal-body');body.innerHTML=`<h2 style="margin-bottom:15px;color:var(--accent);">📊 Statistics</h2><div class="stats-tabs"><button class="stats-tab-btn ${section==='anime'?'active':''}" onclick="renderStats('anime')">🎌 Anime</button><button class="stats-tab-btn ${section==='tv'?'active':''}" onclick="renderStats('tv')">📺 TV</button><button class="stats-tab-btn ${section==='movies'?'active':''}" onclick="renderStats('movies')">🎬 Movies</button></div><div id="stats-body"></div>`;openModal('stats-modal');renderStats(section);}

function renderStats(section){document.querySelectorAll('.stats-tab-btn').forEach(b=>{b.classList.remove('active');if((section==='anime'&&b.textContent.includes('Anime'))||(section==='tv'&&b.textContent.includes('TV'))||(section==='movies'&&b.textContent.includes('Movies')))b.classList.add('active');});const container=document.getElementById('stats-body');if(!container)return;if(section==='movies'){renderMovieStats(container);return;}
const items=section==='anime'?getAnime():getTVShows();const epMin=section==='anime'?24:45;const excludeGenres=section==='anime'?new Set(['Animation']):new Set();
let totalEps=0;const statusCounts={},monthCounts={},dayOfWeekCounts=[0,0,0,0,0,0,0],hourCounts=new Array(24).fill(0),bingeData={};const genreCounts={},networkCounts={},languageCounts={},decadeCounts={};const showSpeeds=[],longestShows=[];let totalRating=0,ratedCount=0;
items.forEach(item=>{statusCounts[item.user_status||'Unknown']=(statusCounts[item.user_status||'Unknown']||0)+1;(item.genres||[]).forEach(g=>{if(excludeGenres.has(g))return;genreCounts[g]=(genreCounts[g]||0)+1;});(item.networks||[]).slice(0,1).forEach(n=>{networkCounts[n]=(networkCounts[n]||0)+1;});if(item.original_language){languageCounts[languageCodeToName(item.original_language)]=(languageCounts[languageCodeToName(item.original_language)]||0)+1;}const yr=item.year;if(yr&&yr>1900){decadeCounts[`${Math.floor(yr/10)*10}s`]=(decadeCounts[`${Math.floor(yr/10)*10}s`]||0)+1;}if(item.tmdb_rating&&item.tmdb_rating>0){totalRating+=item.tmdb_rating;ratedCount++;}
if(['Finished','Up to Date'].includes(item.user_status)&&item.created_at){const lastWatched=getLastWatchedDate(item);const days=Math.round((new Date(lastWatched)-new Date(item.created_at))/86400000);const aired=getAiredEpisodesOnly(item.seasons);if(days>0&&days<3650){showSpeeds.push({title:item.title,days,eps:aired.length});longestShows.push({title:item.title,days,eps:aired.length});}}
item.seasons?.forEach(s=>{if(s.number===0)return;s.episodes?.forEach(ep=>{if(!ep.is_watched||ep.is_special)return;totalEps++;if(ep.watched_at){const d=new Date(ep.watched_at);const key=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`;monthCounts[key]=(monthCounts[key]||0)+1;dayOfWeekCounts[d.getDay()]++;hourCounts[d.getHours()]++;bingeData[d.toISOString().split('T')[0]]=(bingeData[d.toISOString().split('T')[0]]||0)+1;}});});});
const filteredBinge=Object.fromEntries(Object.entries(bingeData).filter(([,c])=>c<=25));let longestStreak=0,currentStreak=0;const watchDays=Object.keys(filteredBinge).sort();watchDays.forEach((day,i)=>{if(i===0)currentStreak=1;else{currentStreak=(new Date(day)-new Date(watchDays[i-1]))/86400000===1?currentStreak+1:1;}if(currentStreak>longestStreak)longestStreak=currentStreak;});
const topBingeDays=Object.entries(filteredBinge).sort((a,b)=>b[1]-a[1]).slice(0,5);showSpeeds.sort((a,b)=>(b.eps/Math.max(b.days,1))-(a.eps/Math.max(a.days,1)));longestShows.sort((a,b)=>b.days-a.days);
...
const twoMonthsAgo=new Date(Date.now()-60*24*60*60*1000);let recentEps=0;items.forEach(item=>{item.seasons?.forEach(s=>{if(s.number===0)return;s.episodes?.forEach(ep=>{if(ep.is_watched&&ep.watched_at&&!ep.is_special&&new Date(ep.watched_at)>=twoMonthsAgo)recentEps++;});});});
const recentPerWeek=Math.round(recentEps/8);const weeksToFinish=recentPerWeek>0?Math.ceil(remaining/recentPerWeek):null;const finishDate=weeksToFinish?new Date(Date.now()+weeksToFinish*7*24*60*60*1000).toLocaleDateString('en-US',{year:'numeric',month:'long',day:'numeric'}):null;
const finishedCount=(statusCounts['Finished']||0)+(statusCounts['Up to Date']||0);const totalStarted=items.filter(i=>i.user_status!=='Planned').length;const dropRate=totalStarted>0?Math.round(((statusCounts['Dropped']||0)/totalStarted)*100):0;const completeRate=totalStarted>0?Math.round((finishedCount/totalStarted)*100):0;
const peakHour=hourCounts.indexOf(Math.max(...hourCounts));const peakPeriod=peakHour<6?'Late Night 🌙':peakHour<12?'Morning ☀️':peakHour<17?'Afternoon 🌤':peakHour<21?'Evening 🌆':'Night 🌃';const dayNames=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];const peakDay=dayNames[dayOfWeekCounts.indexOf(Math.max(...dayOfWeekCounts))];const weekendEps=dayOfWeekCounts[0]+dayOfWeekCounts[6];const weekdayEps=dayOfWeekCounts.slice(1,6).reduce((a,b)=>a+b,0);const watcherType=weekendEps>weekdayEps?'Weekend Watcher 📅':'Weekday Watcher 💼';const activeDays=Object.keys(filteredBinge).length;const avgBinge=activeDays>0?(totalEps/activeDays).toFixed(1):0;const bingeType=avgBinge>=5?'Binge Watcher 🍿':avgBinge>=2?'Casual Watcher 📺':'Light Watcher ☕';
const topGenres=Object.entries(genreCounts).sort((a,b)=>b[1]-a[1]).slice(0,5);const topNetworks=Object.entries(networkCounts).sort((a,b)=>b[1]-a[1]).slice(0,5);const topLanguages=Object.entries(languageCounts).sort((a,b)=>b[1]-a[1]).slice(0,5);const topDecades=Object.entries(decadeCounts).sort((a,b)=>b[1]-a[1]);const avgRating=ratedCount>0?(totalRating/ratedCount).toFixed(1):'N/A';
const topGenreShare=topGenres.length&&items.length?Math.round((topGenres[0][1]/items.length)*100):0;const genreLoyalty=topGenreShare>=60?'Genre Loyalist 🎯':topGenreShare>=40?'Genre Curious 🔍':'Genre Explorer 🌍';
const seasonCounts={Spring:0,Summer:0,Autumn:0,Winter:0};Object.entries(monthCounts).forEach(([key,count])=>{const month=parseInt(key.split('-')[1]);if(month>=3&&month<=5)seasonCounts.Spring+=count;else if(month>=6&&month<=8)seasonCounts.Summer+=count;else if(month>=9&&month<=11)seasonCounts.Autumn+=count;else seasonCounts.Winter+=count;});const topSeason=Object.entries(seasonCounts).sort((a,b)=>b[1]-a[1])[0]?.[0]||'—';
const completedWithPop=items.filter(i=>['Finished','Up to Date'].includes(i.user_status)&&i.popularity).sort((a,b)=>a.popularity-b.popularity);const rarestWatch=completedWithPop[0]?.title||'—';
const simultaneouslyWatching=items.filter(i=>i.user_status==='Watching'||(i.user_status==='Up to Date'&&isCurrentlyAiring(i))).length;

container.innerHTML=`<div class="stats-card"><h4>📈 Overview</h4><div class="stats-row"><span class="stats-label">Total</span><span class="stats-value">${items.length}</span></div><div class="stats-row"><span class="stats-label">Episodes</span><span class="stats-value">${totalEps.toLocaleString()}</span></div><div class="stats-row"><span class="stats-label">Watch Time</span><span class="stats-value">${formatWatchTime(totalEps*epMin)}</span></div><div class="stats-row"><span class="stats-label">Completed</span><span class="stats-value">${finishedCount}</span></div><div class="stats-row"><span class="stats-label">In Progress</span><span class="stats-value">${statusCounts['Watching']||0}</span></div><div class="stats-row"><span class="stats-label">Simultaneously</span><span class="stats-value">${simultaneouslyWatching}</span></div><div class="stats-row"><span class="stats-label">Complete Rate</span><span class="stats-value">${completeRate}%</span></div><div class="stats-row"><span class="stats-label">Drop Rate</span><span class="stats-value">${dropRate}%</span></div><div class="stats-row"><span class="stats-label">Avg Rating</span><span class="stats-value">⭐${avgRating}</span></div></div>
<div class="stats-card"><h4>⚡ Speed</h4><div class="stats-row"><span class="stats-label">Eps/Month</span><span class="stats-value">${avgPerMonth}</span></div><div class="stats-row"><span class="stats-label">Eps/Week</span><span class="stats-value">${avgPerWeek}</span></div><div class="stats-row"><span class="stats-label">Recent</span><span class="stats-value">${recentPerWeek}/wk</span></div><div class="stats-row"><span class="stats-label">Streak</span><span class="stats-value">${longestStreak}d</span></div></div>
<div class="stats-card"><h4>📋 Backlog</h4><div class="stats-row"><span class="stats-label">Remaining</span><span class="stats-value">${remaining.toLocaleString()}</span></div><div class="stats-row"><span class="stats-label">Time Left</span><span class="stats-value">${formatWatchTime(remaining*epMin)}</span></div><div class="stats-row"><span class="stats-label">Pace</span><span class="stats-value">${recentPerWeek>0?`${weeksToFinish}wk`:'∞'}</span></div><div class="stats-row"><span class="stats-label">Catch-Up</span><span class="stats-value">${finishDate||'—'}</span></div></div>
<div class="stats-card"><h4>🧠 Habits</h4><div class="stats-row"><span class="stats-label">Type</span><span class="stats-value">${watcherType}</span></div><div class="stats-row"><span class="stats-label">Style</span><span class="stats-value">${bingeType}</span></div><div class="stats-row"><span class="stats-label">Avg/Day</span><span class="stats-value">${avgBinge}</span></div><div class="stats-row"><span class="stats-label">Peak</span><span class="stats-value">${peakPeriod}</span></div><div class="stats-row"><span class="stats-label">Day</span><span class="stats-value">${peakDay}</span></div><div class="stats-row"><span class="stats-label">Season</span><span class="stats-value">${topSeason}</span></div><div class="stats-row"><span class="stats-label">Weekend</span><span class="stats-value">${weekendEps.toLocaleString()}</span></div><div class="stats-row"><span class="stats-label">Weekday</span><span class="stats-value">${weekdayEps.toLocaleString()}</span></div></div>
<div class="stats-card"><h4>🎭 Content</h4><div class="stats-row"><span class="stats-label">Genre Style</span><span class="stats-value">${genreLoyalty}</span></div><div class="stats-row"><span class="stats-label">Rarest</span><span class="stats-value" style="font-size:11px;max-width:160px;text-align:right;">${rarestWatch}</span></div>${topGenres.map(([g,c])=>`<div class="stats-row"><span class="stats-label">🎬${g}</span><span class="stats-value">${c}</span></div>`).join('')}</div>
...
${topBingeDays.length?`<div class="stats-card"><h4>🍿 Binges</h4>${topBingeDays.map(([date,count],i)=>`<div class="stats-row"><span class="stats-label">${i+1}. ${new Date(date).toLocaleDateString('en-US',{weekday:'short',month:'short',day:'numeric',year:'numeric'})}</span><span class="stats-value">${count}ep</span></div>`).join('')}</div>`:''}
...
const dc=document.getElementById('stats-dow-chart');if(dc){new Chart(dc.getContext('2d'),{type:'bar',data:{labels:dayNames,datasets:[{label:'Eps',data:dayOfWeekCounts,backgroundColor:['#FF6384','#36A2EB','#FFCE56','#4BC0C0','#9966FF','#FF9F40','#FF6384'],borderWidth:1}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true}}}});}
...
function renderMovieStats(container){const movies=getMovies();const twoMonthsAgo=new Date(Date.now()-60*24*60*60*1000);const watched=movies.filter(m=>m.is_watched);const rewatched=movies.reduce((s,m)=>s+(m.rewatch_count||0),0);const monthCounts={},dayOfWeekCounts=[0,0,0,0,0,0,0],genreCounts={},languageCounts={},decadeCounts={};let totalRating=0,ratedCount=0;
...
const monthKeys=Object.keys(monthCounts).sort();const avgPerMonth=monthKeys.length?(watched.length/monthKeys.length).toFixed(1):0;const dayNames=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];const peakDay=dayNames[dayOfWeekCounts.indexOf(Math.max(...dayOfWeekCounts))];const unwatched=movies.filter(m=>!m.is_watched).length;const avgRating=ratedCount>0?(totalRating/ratedCount).toFixed(1):'N/A';const topGenres=Object.entries(genreCounts).sort((a,b)=>b[1]-a[1]).slice(0,5);const topLanguages=Object.entries(languageCounts).sort((a,b)=>b[1]-a[1]).slice(0,5);const topDecades=Object.entries(decadeCounts).sort((a,b)=>b[1]-a[1]);const recentWatched=movies.filter(m=>m.is_watched&&m.watched_at&&new Date(m.watched_at)>=twoMonthsAgo).length;const recentPerWeek=(recentWatched/8).toFixed(1);const weeksToFinish=recentWatched>0?Math.ceil(unwatched/(recentWatched/8)):null;const finishDate=weeksToFinish?new Date(Date.now()+weeksToFinish*7*24*60*60*1000).toLocaleDateString('en-US',{year:'numeric',month:'long'}):null;
container.innerHTML=`<div class="stats-card"><h4>📈 Overview</h4><div class="stats-row"><span class="stats-label">Total</span><span class="stats-value">${movies.length}</span></div><div class="stats-row"><span class="stats-label">Watched</span><span class="stats-value">${watched.length}</span></div><div class="stats-row"><span class="stats-label">Unwatched</span><span class="stats-value">${unwatched}</span></div><div class="stats-row"><span class="stats-label">Rewatched</span><span class="stats-value">${rewatched}</span></div><div class="stats-row"><span class="stats-label">Time</span><span class="stats-value">${formatWatchTime(watched.length*100)}</span></div><div class="stats-row"><span class="stats-label">Avg/Mo</span><span class="stats-value">${avgPerMonth}</span></div><div class="stats-row"><span class="stats-label">Day</span><span class="stats-value">${peakDay}</span></div><div class="stats-row"><span class="stats-label">Rating</span><span class="stats-value">⭐${avgRating}</span></div></div>
<div class="stats-card"><h4>📋 Backlog</h4><div class="stats-row"><span class="stats-label">Left</span><span class="stats-value">${unwatched}</span></div><div class="stats-row"><span class="stats-label">Pace</span><span class="stats-value">${recentPerWeek}/wk</span></div><div class="stats-row"><span class="stats-label">Catch-Up</span><span class="stats-value">${finishDate||'—'}</span></div></div>
...
const dc=document.getElementById('stats-dow-chart');if(dc){new Chart(dc.getContext('2d'),{type:'bar',data:{labels:dayNames,datasets:[{label:'Movies',data:dayOfWeekCounts,backgroundColor:'rgba(156,39,176,0.4)',borderColor:'rgba(156,39,176,1)',borderWidth:1}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true}}}});}
...
function exportToTrakt() {
    const traktData = [];
    const movies = getMovies();
    const shows  = [...getAnime(), ...getTVShows()];

    // Movies
    movies.forEach(m => {
        if (m.is_watched && m.watched_at) {
            traktData.push({
                tmdb_id: String(m.tmdb_id || ''),
                imdb_id: m.imdb_id || undefined,
                type: 'movie',
                watched_at: m.watched_at,
                rating: m.my_rating || undefined,
                rated_at: m.my_rating ? m.watched_at : undefined
            });
        }
        // Rewatches
        if (m.rewatch_history?.length) {
            m.rewatch_history.forEach(rw => {
                traktData.push({
                    tmdb_id: String(m.tmdb_id || ''),
                    imdb_id: m.imdb_id || undefined,
                    type: 'movie',
        ...
        // Watchlisted (unwatched)
        if (!m.is_watched && m.created_at) {
            traktData.push({
                tmdb_id: String(m.tmdb_id || ''),
                imdb_id: m.imdb_id || undefined,
                type: 'movie',
                watchlisted_at: m.created_at,
                rating: m.my_rating || undefined,
                rated_at: m.my_rating ? m.created_at : undefined
            });
        }
    });

    // TV Shows — export each watched episode
    shows.forEach(show => {
        // Show-level rating
        if (show.my_rating && show.tmdb_id) {
            traktData.push({
                tmdb_id: String(show.tmdb_id),
                imdb_id: show.imdb_id || undefined,
                type: 'show',
                rating: show.my_rating,
                rated_at: show.created_at || new Date().toISOString()
            });
        }

        // Watchlisted (planned)
        if (show.user_status === 'Planned' && show.created_at) {
            traktData.push({
                tmdb_id: String(show.tmdb_id || ''),
                imdb_id: show.imdb_id || undefined,
                type: 'show',
                watchlisted_at: show.created_at
            });
        }

        // Each watched episode
        show.seasons?.forEach(s => {
            ...
            s.episodes?.forEach(ep => {
                ...
                // Rewatches for this episode
                if (ep.rewatch_history?.length) {
                    ep.rewatch_history.forEach(rw => {
                        traktData.push({
                            tmdb_id: String(show.tmdb_id || ''),
                            imdb_id: show.imdb_id || undefined,
                            type: 'episode',
    ...
    const cleaned = traktData.map(item => {
        const clean = {};
...
function openPersonalListModal(){
    ...
    document.getElementById('personal-list-body').innerHTML=`
        ...
        <div style="display:flex;gap:12px;margin-bottom:16px;flex-wrap:wrap;">
            ...
            <label style="display:flex;align-items:center;gap:6px;font-size:14px;cursor:pointer;"><input type="checkbox" id="pl-tv" checked> TV</label>
...
function generatePersonalList(){
    ...
    if(!includeAnime&&!includeTv&&!includeMovies){alert('Select a type.');return;}if(!inclWatched&&!inclPlanned&&!inclPaused&&!inclDropped){alert('Select a status.');return;}
    ...
    if(isMovie){if(inclWatched){const w=items.filter(i=>i.is_watched&&!i.hide_from_list);if(w.length)groups['Watched']=w;}const uw=items.filter(i=>!i.is_watched&&!i.hide_from_list);if(inclPlanned&&uw.length)groups['Planned']=uw;}
...
window.syncAllAiringShows=syncAllAiringShows;window.enrichLibrary=enrichLibrary;window.fetchAllAirTimes=fetchAllAirTimes;
...
window.closeModal=closeModal;window.refreshApp=refreshApp;window.generateErrorLog=generateErrorLog;
